...
![Kiến trúc hệ thống YAS](image.png)
...
5. Tạo Jenkins job để xóa phần triển khai ở mục 4. (Tham khảo: <https://community.jenkins.io/t/how-to-add-hyperlink-using-jenkins-job-builder/7091>)
...
6. Bỏ qua phần này nếu làm phần Nâng Cao: Tương tự, trên Jenkins tạo ra 2 job CI/CD để deploy `dev` và `staging`.
   ...
   - **b.** Staging: trên `main` branch sẽ có các tag để có dạng release, ví dụ: tag `v1.2.3`. Thì job CI/CD sẽ phát hiện và build image với tag cuối cùng, ví dụ tag `v1.2.3` (hoặc tách branch `rc_v1.2.3`, hoặc vừa tag và tách branch), sau đó push images này lên Docker Hub và deploy vào trong namespace `staging`.

### Nâng cao (2đ): Sử dụng ArgoCD để handle được `dev` và `staging`

![ArgoCD flow](image-2.png)
...
2. Nộp bài: Các bạn tạo file báo cáo gồm các thông tin sau:
   ...
   - **b.** Đặt tên file theo format `<MSSV1>_<MSSV2>_<MSSV3>.docx`. Thứ tự MSSV cần được sắp xếp tăng dần.
